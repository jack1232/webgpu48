...
export const ParametricSurfaceData = (f:any, umin:number, umax:number, vmin:number, vmax:number, nu:number, nv:number, 
    xmin:number, xmax:number, zmin:number, zmax:number, scale = 1, scaley = 0, colormapName = 'jet', center:vec3=[0,0,0], ul = 1, vl = 1) =>{
    if(nu<2 || nv<2) return;
...
const NormalizeUVParametric = (u:number, v:number, umin:number, umax:number, vmin:number, vmax:number, ul = 1, vl = 1) => {
